"""
FORWARD-TEST SCELLÉ s20 — CLI du pas de mesure
===============================================

    python -m studies.s20_forward.run_forward [--stale-ok]

Exécutable à toute fréquence (une fois par heure — barres H1) :
...
PARAMS_SHA256 = "5fd385aae7af0ad59b98bd44e44bbcc8b458624fc7f2998f5d26302843419290"

# Cache court : un pas de mesure horaire (barres H1) doit voir des barres fraîches.
FRESH_MAX_AGE_H = 1
# EMA 26 + signal 9 + warmup S020 (60) + marge : en dessous, la cellule n'a
# même pas son indicateur — refuser de mesurer plutôt que mesurer faux.
WARMUP_MIN_BARS = 150


def make_signal_fn(params: dict, symbol: str):
    """La stratégie S020 avec les paramètres SCELLÉS, et rien d'autre.

    Les signaux sont recalculés sur l'historique complet à chaque passage —
    même code que le backtest (R5 : pas de deuxième
    implémentation ; `strategy.precompute` + `generate_signals`, exactement
    comme `backtests/run_wf.py`). Seuls les signaux postérieurs au
...
def main(argv: list[str]) -> int:
    ...
    paths = Paths()
    if first_pass_refused(paths.journal, "s20_forward"):
        return 2
