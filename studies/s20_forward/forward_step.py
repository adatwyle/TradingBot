"""
FORWARD-TEST SCELLÉ s20 — LE PAS DE MESURE (bibliothèque)
==========================================================
...
instruments : EURUSD (bras PRINCIPAL — seul comptable dans les critères
d'arrêt) et USDJPY (bras d'OBSERVATION — journalisé à l'identique, jamais
...
`engine_kwargs` DE L'ÉTUDE S020 (cooldown 2, circuit breaker 3 pertes → 24
barres — les règles communes contre les pertes consécutives, les mêmes que
`strategies/S020_balke_macd_cross/backtests/run_wf.py`) :
...
def default_data_dir() -> str:
    env = os.environ.get("S20_FORWARD_DIR")
    return env if env else os.path.join(str(db_dir()), "s20_forward")


JOURNAL_COLUMNS = [
    "measured_at_utc",   # quand le script a tourné — l'horodatage de MESURE
    "event",             # OPEN | CLOSE
    "arm",               # PRIMARY (EURUSD) | OBSERVATION (USDJPY)
    "symbol",
    "trade_id",
...
def run_step(dfs: dict[str, pd.DataFrame], params: dict, paths: Paths,
    ...
    {bar_time -> Signal} — en production la stratégie S020 avec les paramètres
