"""
Tests du pas de mesure s20 — sur barres synthétiques, sans MT5.

    python -m pytest studies/s20_forward/test_forward_step.py -q
...
même barre autorisée avec cooldown 0 (engine_kwargs de TEST — la mécanique ;
l'étude réelle scelle cooldown 2 et coupe-circuit 3 → 24 dans params.json).
"""
...
from studies.s20_forward.forward_step import (                     # noqa: E402
...
PARAMS = {
    ...
    "instruments": {"EURUSD": "PRIMARY", "USDJPY": "OBSERVATION"},
    "specs": {"EURUSD": {"symbol": "EURUSD", **SPEC},
              "USDJPY": {"symbol": "USDJPY", **SPEC}},
    # engine_kwargs de TEST : cooldown 0, circuit breaker désarmé (mécanique pure).
...
def bars(rows, start="2026-08-17"):
    """rows = [(open, high, low, close), ...] -> DataFrame (barres synthétiques)."""
...
def sig_at(df, i, symbol="EURUSD", side=Side.LONG, sl=1.5, tp=6.0):
...
NO_SIG = {"EURUSD": no_signals, "USDJPY": no_signals}


def fns(aud=None, eur=None):
    return {"EURUSD": (lambda d: aud) if aud is not None else no_signals,
            "USDJPY": (lambda d: eur) if eur is not None else no_signals}
...
def seal_then_extend(paths, df_seal, dfs_full, signal_fns):
    ...
    s0 = run_step({"EURUSD": df_seal, "USDJPY": df_seal}, PARAMS, paths, NO_SIG)
...
def test_idempotence_meme_donnees_aucun_ajout(paths):
    ...
    both = {"EURUSD": df_full, "USDJPY": df_full}
...
def test_premier_passage_exige_les_deux_flux(paths):
    ...
    with pytest.raises(ValueError):
        run_step({"EURUSD": flat(10)}, PARAMS, paths, NO_SIG)
...
def test_reecriture_du_journal_detectee_et_refus(paths):
    ...
    both = {"EURUSD": flat(20), "USDJPY": flat(20)}
...
def test_hash_du_vrai_fichier_scelle_correspond():
    ...
    from studies.s20_forward.run_forward import PARAMS_PATH, PARAMS_SHA256
...
def test_stop_saute_par_gap_execute_a_l_ouverture(paths):
    ...
    paie — même règle que le moteur commun (correctif 66668d1). Sur ces barres les
    ...
    assert close_row["symbol"] == "EURUSD" and close_row["arm"] == "PRIMARY"
...
def test_take_profit_et_monnaie_coherente(paths):
    ...
    assert st["arms"]["USDJPY"]["capital"] == pytest.approx(10000.0)
...
def test_position_unique_et_reentree_meme_barre_cooldown_zero(paths):
    ...
    l'étude s20 en test), un signal sur la barre de SORTIE ré-entre le jour même —
...
def test_flux_absent_curseur_intact_l_autre_avance(paths):
    ...
    df_seal = flat(10)
    df_full = flat(15)
    sig_eur = sig_at(df_full, 11, symbol="USDJPY")

    run_step({"EURUSD": df_seal, "USDJPY": df_seal}, PARAMS, paths, NO_SIG)

    # Passage avec USDJPY absent : EURUSD avance, USDJPY reste au scellé.
    st = run_step({"EURUSD": df_full}, PARAMS, paths, fns(eur=sig_eur))
    assert st["arms"]["EURUSD"]["last_bar_time"] == df_full.index[-1].isoformat()
    assert st["arms"]["USDJPY"]["last_bar_time"] == df_seal.index[-1].isoformat()
    assert st["opened_this_pass"] == 0

    # USDJPY revient : ses barres manquées sont rejouées, le signal est pris.
    st2 = run_step({"EURUSD": df_full, "USDJPY": df_full}, PARAMS, paths,
                   fns(eur=sig_eur))
    assert st2["opened_this_pass"] == 1
    opens = [r for r in read_journal(paths.journal) if r["event"] == "OPEN"]
    assert len(opens) == 1
    assert opens[0]["symbol"] == "USDJPY" and opens[0]["arm"] == "OBSERVATION"


def test_observation_n_entre_pas_dans_la_comptabilite_du_principal(paths):
    """Un trade USDJPY (OBSERVATION) est journalisé à l'identique mais ne
    touche NI le n_closed NI le capital du bras PRINCIPAL — c'est ce qui rend
    les critères d'arrêt insensibles à l'observation."""
    df_seal = flat(10)
    rows = [(100.0, 100.2, 99.8, 100.0)] * 11 + [
        (100.0, 100.3, 99.9, 100.0),          # 11 : SIGNAL USDJPY
    ...
    assert st["arms"]["USDJPY"]["n_closed_total"] == 1
    assert st["arms"]["USDJPY"]["capital"] < 10000.0
    assert st["arms"]["EURUSD"]["n_closed_total"] == 0
    assert st["arms"]["EURUSD"]["capital"] == pytest.approx(10000.0)
