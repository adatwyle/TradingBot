"""
ESSAI À BLANC « DAILY MACD ASSISTÉ PAR IA » — LE PAS DE MESURE (bibliothèque)
==============================================================================

Logique pure du pas quotidien, séparée de l'accès MT5 et du CLI `claude` pour
...
CONTEXTE — le socle mécanique est MESURÉ MORT (s12 VERDICT, commit a28cf16)
----------------------------------------------------------------------------
Le bras « prendre-tout » rejoue une stratégie dont le verdict mécanique est
PAS D'EDGE (−0,006 R/trade à spread nul, percentile 51,5). La question testée
ici n'est PAS « la stratégie marche-t-elle » mais : « une session Claude Code
peut-elle extraire un sous-ensemble profitable d'un flux de candidats mesuré
non profitable ? » — s93 a répondu NON sur un autre flux ; ce dispositif doit
pouvoir rendre le même verdict honnêtement. Voir PROTOCOL.md.

LES TROIS BRAS — journalisés à CHAQUE signal (PROTOCOL.md § 3)
---------------------------------------------------------------
    MECH  la stratégie nue : prend tout signal quand elle est flat.
    AI    la session Claude Code headless décide (take/skip + taille 0..1 +
          SL/TP resserrés DANS les bornes). Panne du CLI -> décision « na » :
          le bras IA saute CE signal, les témoins continuent. Une panne d'IA
          ne fausse JAMAIS les témoins.
    ...
    position contrefactuelle à configuration de base, SANS blocage de position
    ni cooldown. C'est elle qui fournit le R par signal dont la falsification
    F1 (sélection IA vs tirages aléatoires) et F2 (socle à coût nul) ont
    besoin, indépendamment de l'état des comptes.

MULTI-INSTRUMENTS
-----------------
Trois flux D1 scellés (SP500, NASDAQ, DAX — ~12 signaux/an au lieu de ~4 sur
SP500 seul ; le calcul de fréquence est dans PROTOCOL.md). Un curseur par
instrument, une position au plus par instrument et par bras, capital PARTAGÉ
par bras (pire cas : 3 stops touchés = 3 × le risque unitaire — déclaré).
...
----------------------------------------------------------------------------
Identiques ligne à ligne à `core/backtest/engine.py` (et à gold_forward) :
entrée au close de la barre de signal + coût de bord ; stop UNILATÉRAL, gap
payé à l'ouverture ; la cible ne profite pas du gap ; SL prime sur TP dans la
même barre ; coût de bord aussi à la sortie ; une position par bras et par
instrument, cooldown 2 barres, circuit breaker 3 pertes -> 24 barres (par
instrument). Les trois bras utilisent le MÊME moteur : seule la décision
d'entrée (et la taille/les niveaux pour l'IA) diffère.

SIZING
------
1 % de risque de base par trade sur un capital virtuel initial de 10 000 par
bras, dimensionné par `core/risk/guards.py` (RiskLayer.evaluate — l'IA propose,
la couche risque dispose : défense en profondeur derrière le clamp des bornes).
"""
...
def default_data_dir() -> str:
    env = os.environ.get("MACD_AI_PAPER_DIR")
    return env if env else os.path.join(str(db_dir()), "macd_ai_paper")
...
JOURNAL_COLUMNS = [
    ...
    "sl_adjust",         # DECISION IA : facteur distance de stop (clampé)
    "tp_adjust",         # DECISION IA : facteur distance de cible (clampé)
...
def load_sealed_params(path: str, expected_sha256: str) -> dict:
    ...
    if got != expected_sha256:
        raise SealError(
            ...
            f"des paramètres invalide l'essai (PROTOCOL.md, critère "
            f"d'invariance). Refus de tourner.")
...
def clamp_decision(dec: dict, bounds: dict) -> dict:
    ...
    profondeur n°1 (le prompt interdit déjà de sortir des bornes, RiskLayer
    borne encore le risque monétaire derrière)."""
    def _f(v, lo, hi, dflt):
        try:
            x = float(v)
...
def _open_position(sig: Signal, bar_time: pd.Timestamp, spec: dict,
    ...
    `size_frac` module le budget de risque (IA : 0..1 du 1 % de base).
    `sl_adjust`/`tp_adjust` resserrent la distance stop/cible (facteur <= 1).
    ...
    # Convention moteur : à adjust = 1.0 les niveaux de la stratégie sont
    # utilisés TELS QUELS (le coût de bord vit dans la distance de risque,
    # comme engine.py / gold_forward). Un adjust < 1 resserre la distance
    # mesurée depuis le prix d'entrée EXÉCUTÉ.
...
def build_dossier(df: pd.DataFrame, ts: pd.Timestamp, sig: Signal,
                  params: dict, state: dict, *, n_bars: int = 30) -> dict:
    """Le dossier de décision : contexte de marché FACTUEL, état du compte IA,
    règles de la stratégie, bornes. Barres normalisées en % de l'entrée, pas
    de ticker ni de date : même hygiène que le rejeu à l'aveugle, et le
    dossier reste compact."""
    ...
    stop_dist = abs(entry - float(sig.stop))
    tgt_dist = (abs(float(sig.target) - entry)
                if sig.target is not None else None)
    ai = state["arms"]["AI"]
    return {
        "strategy": {
            "id": params["strategy_id"],
            "rules": ("LONG only, indice actions D1, mean reversion : "
                      "MACD(12,26) < 0 et en baisse 5 jours consécutifs ; "
                      "close < close[veille] ; close dans le bas du range "
                      "20 jours (<= 20 %) ; sortie au dépassement du plus "
                      "haut de la veille (cible statique) ; stop 3 ATR."),
        },
        "signal": {
            "side": sig.side.name,
            "stop_distance_pct_of_entry": round(100 * stop_dist / entry, 3),
            "target_distance_pct_of_entry": (round(100 * tgt_dist / entry, 3)
                                             if tgt_dist else None),
            "reward_risk": (round(tgt_dist / stop_dist, 3) if tgt_dist else None),
            "macd_pct_of_entry": (round(100 * float(meta["macd"]) / entry, 4)
                                  if meta.get("macd") is not None else None),
            "range_pos": meta.get("range_pos"),
            "atr_pct_of_entry": (round(100 * float(meta["atr"]) / entry, 3)
                                 if meta.get("atr") else None),
        },
    ...
    if state is None:
        ...
        if missing:
            raise ValueError(f"premier passage : instruments manquants "
                             f"{missing} — le scellé se pose sur les trois "
                             f"flux à la fois")
...
def _step_symbol(sym: str, df: pd.DataFrame, params: dict, state: dict,
    ...
    for ts, bar in new_bars.iterrows():
        ...
        if sig is not None and spec["spread_pips"] <= spec["max_spread_pips"]:
            signal_id = f"{sym}_{ts.strftime('%Y%m%d')}"
    ...
    status = {
        ...
        "stop_criteria": {
            "fail_ai": (f"actif dès {rules['fail_ai']['min_decisions']} "
                        f"décisions IA (n = {a['n_decisions']}) ; verdict au "
                        f"percentile sélection < "
                        f"{rules['fail_ai']['percentile_below']} — voir "
                        f"report_paper.py"),
